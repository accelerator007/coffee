-- 0009_admin_internal_auth_users.sql
-- Supabase Auth Admin API can fail on this project with "Database error
-- creating new user". This function creates the same email/password Auth rows
-- directly, lets the existing Auth -> profiles trigger run, and returns the
-- created user id to trusted server code.
